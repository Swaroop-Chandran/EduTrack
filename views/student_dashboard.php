...
<?php elseif ($currentTab === 'profile'): 
    ...
    if (isset($_SESSION['profile_edit_failed']) || ($forceProfileCompletion ?? false)) {
...
?>
    ...
    <div class="max-w-4xl mx-auto space-y-6 animate-fadeIn text-left">
        ...
        <div id="profile_edit_panel" class="hidden">
            <?php if ($forceProfileCompletion ?? false): ?>
                <div class="p-4 mb-6 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-xs flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="font-semibold leading-relaxed text-slate-600">Please fill out all personal, parent, and contact details below, and upload any required certificates. Your dashboard access will remain locked until this profile form is completed and saved.</p>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <form action="actions.php?action=edit_profile" method="POST" enctype="multipart/form-data" onsubmit="return validateProfileForm(event);" class="space-y-6">
                ...
                <div class="bg-white border border-[#E2E8F0] p-6 rounded-xl shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label for="edit_personal_email" class="block text-[10px] font-bold text-[#64748B] uppercase tracking-wider mb-1.5">Personal Contact Email</label>
                            <input type="email" name="personal_email" id="edit_personal_email" value="<?php echo htmlspecialchars($studentProfile['personal_email'] ?? ''); ?>" placeholder="e.g. personal@gmail.com" class="w-full px-3 py-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-xs font-semibold text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:border-transparent transition-all font-mono" />
                        </div>
                        <div class="md:col-span-2">
                ...
                <div class="flex justify-end gap-3 font-bold text-xs pt-4 border-t border-[#E2E8F0]">
                    ...
                    <button type="submit" class="py-2.5 px-6 bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-lg shadow-sm cursor-pointer transition-colors">
                        Save Profile & Complete Activation
                    </button>
                </div>
            </form>
        </div>
